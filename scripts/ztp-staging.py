...
def _inject_cmd_vrf_into_ztp_script(hostname: str, vrf: str) -> None:
    """
    BCM 10 template consumes CMD_VRF, but BCM 10 does not inject it. Patch the generated per-switch
    script after initialize so future ZTP runs use the correct VRF (e.g., for cm-lite-daemon register).

    Note: Any later `device initialize` will regenerate the script; ztp-staging calls initialize, so
    we re-apply this patch each staging run.
    """
    vrf = (vrf or "").strip()
    if not vrf:
        return
    ...
    if not ztp_script.exists():
        return
    txt = ztp_script.read_text(errors="replace").splitlines(True)
    ...
    for i, line in enumerate(txt):
        if "BEGIN AUTOGENERATED SECTION" in line:
            begin = i
        if "END AUTOGENERATED SECTION" in line and begin is not None and i > begin:
            end = i
            break
    if begin is None or end is None:
        return

    # Update existing CMD_VRF if present inside the autogen block.
    for i in range(begin, end):
        if txt[i].startswith("CMD_VRF=") or txt[i].startswith("CMD_VRF'") or txt[i].startswith("CMD_VRF\""):
            txt[i] = f"CMD_VRF='{vrf}'\n"
            ztp_script.write_text("".join(txt))
            return
        if txt[i].startswith("CMD_VRF="):
            txt[i] = f"CMD_VRF='{vrf}'\n"
            ztp_script.write_text("".join(txt))
            return
        if txt[i].startswith("CMD_VRF"):
            # Catch variants like CMD_VRF='...'
            txt[i] = f"CMD_VRF='{vrf}'\n"
            ztp_script.write_text("".join(txt))
            return

    # Otherwise insert near other CMD_* variables (right after CMD_HOSTNAME if present), else at end of block.
    insert_at = end
    for i in range(begin, end):
        if txt[i].startswith("CMD_HOSTNAME="):
            insert_at = i + 1
            break
    txt.insert(insert_at, f"CMD_VRF='{vrf}'\n")
    ztp_script.write_text("".join(txt))
