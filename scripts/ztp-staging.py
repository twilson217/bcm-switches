...
def _read_ztp_config() -> Dict[str, str]:
    """
    Read persistent ZTP settings (currently: vrf) written by deploy_bcm_switches.py.

    This file is intended to persist even when config.json is deleted at the end of a successful run.
    """
    if not ZTP_CONFIG_FILE.exists():
        return {}
    try:
        data = json.loads(ZTP_CONFIG_FILE.read_text())
...
def _cmsh_set_ztp_keyvalue(hostname: str, key: str, value: str) -> None:
    """
    Set a ZTP key/value setting under `device -> ztpsettings -> keyvaluesettings`.

    Rationale:
    - The per-switch `/cm/local/apps/cmd/etc/htdocs/switch/<switch>/cumulus-ztp.sh` is *generated*
      from the template during `device initialize`, and any manual edits to the generated script
      are overwritten on the next initialize.
    - `ztpsettings->keyvaluesettings` is persisted in BCM and is injected into the generated script
      during initialize, making it the correct mechanism for BCM10.
    """
    key = (key or "").strip()
    value = (value or "").strip()
    if not key or not value:
        return
    # Keep this conservative: cmsh is not a shell, but whitespace/quotes in keys/values make
    # parsing ambiguous. VRF names should be simple tokens anyway.
    if re.search(r"\s", key) or re.search(r"\s", value):
        raise ValueError(f"Invalid key/value for ztpsettings keyvaluesettings: {key}={value!r}")

    r = _run_cmsh(f"device; use {hostname}; ztpsettings; keyvaluesettings; set {key} {value}; commit")
    if r.returncode != 0:
        raise RuntimeError(r.stderr.strip() or r.stdout.strip() or f"cmsh set ztpsettings keyvaluesettings {key} failed")


def _cmsh_set_vrf(hostname: str, vrf: str) -> None:
    """
    BCM 11 supports `device.vrf`. BCM 10 does not (per lab observation).
    """
    vrf = (vrf or "").strip()
    if not vrf:
        return
    ...
    if r.returncode != 0:
        raise RuntimeError(r.stderr.strip() or r.stdout.strip() or "cmsh set vrf failed")


def _cmsh_get_vrf(hostname: str) -> str:
    r = _run_cmsh(f"device; use {hostname}; get vrf")
    if r.returncode != 0:
        return ""
...
def main() -> int:
    ...
    # Load persistent ZTP settings (currently: vrf).
    ztp_cfg = _read_ztp_config()
    ztp_vrf = (ztp_cfg.get("vrf") or "").strip()
    if ztp_vrf:
        print(f"ZTP config: vrf='{ztp_vrf}' (from {ZTP_CONFIG_FILE})")
    else:
        print(f"ZTP config: vrf not set (no {ZTP_CONFIG_FILE}); proceeding without explicit VRF injection")
    ...
    for i, dev in enumerate(devices, 1):
        ...
        try:
            ...
            if ztp_vrf and getattr(props, "major", 0) >= 11:
                ...
                if after != ztp_vrf:
                    print(f"  - WARNING: BCM vrf did not persist (before='{before}', after='{after}', expected='{ztp_vrf}')")
                else:
                    print(f"  - BCM: set vrf={after} (commit)")
            # BCM 10: use ztpsettings->keyvaluesettings so CMD_VRF is injected during initialize.
            # (Manual edits to the generated per-switch cumulus-ztp.sh are overwritten by initialize.)
            if ztp_vrf and getattr(props, "major", 0) < 11:
                _cmsh_set_ztp_keyvalue(dev["hostname"], "CMD_VRF", ztp_vrf)
                print(f"  - BCM: set ztpsettings keyvaluesettings CMD_VRF={ztp_vrf} (commit)")
