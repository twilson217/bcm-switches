...
if ENV_FILE.exists():
    ...
    print("Please copy sample-configs/sample.env to .env and fill in your credentials")
    sys.exit(1)

# Get credentials
api_token = os.getenv('AIR_API_TOKEN')
username = os.getenv('AIR_USERNAME', 'user@example.com')
api_base_url = os.getenv('AIR_API_URL', 'https://air.nvidia.com')

if not api_token or api_token.endswith('_here'):
    print("Error: AIR_API_TOKEN not set in .env")
    sys.exit(1)
